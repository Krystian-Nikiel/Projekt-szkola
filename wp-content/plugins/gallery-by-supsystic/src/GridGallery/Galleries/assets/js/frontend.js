(function ($, undefined) {
    ...
    Gallery.prototype.initCaptionEffects = (function () {
        ...
        $.each(this.$elements, function(index, el) {
            ...
            if(!$el.find('.hi-icon-wrap').length){
                ...
                caption_text = caption_text.toString().replace(/<a(.*?)>(.*?)<\/a>/gi,"<object type='none'>$&</object>");
                if(caption_span){
                    caption_place.html('<span '+caption_span.attr('style')+'>'+caption_text+'</span>');
                } else {
                    caption_place.html(caption_text);
                }
            }
    ...
    Gallery.prototype.hideTitleTooltip = (function () {
        if(this.$container.data('hide-tooltip') == true) {
            title = '';
            this.$container.find('a, img').on('mouseenter', function() {
